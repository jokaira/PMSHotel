...
def crear_tarjetas_kpi(master, dict):
    for kpi, data in dict.items():
        contenedor = ctk.CTkFrame(master=master, height=109, fg_color='transparent')
        contenedor.grid(row = 0, column = data['col'], sticky = 'nsew', padx = 12, pady = 12)
        contenedor.grid_propagate(False)

        tarjeta = ctk.CTkFrame(contenedor, fg_color=data['color'], corner_radius=14)
        tarjeta.place(x=0, y=0, relwidth = 1, relheight=1)
...
class Boton(ctk.CTkButton):
        def __init__(self, master, texto, color = AZUL, hover = AZUL2, padx = (0, 12), fill = 'y', pady = 0, tamano_texto = TAMANO_TEXTO_DEFAULT, altura = 37, metodo = ''):
            super().__init__(master = master, 
                            text = texto, 
                            fg_color=color, 
                            hover_color=hover,
                            text_color=BLANCO,font=(FUENTE, tamano_texto),
                            corner_radius=10, 
                            height=altura,
                            command=metodo
                            )
            self.pack(side='left',fill = fill, padx=padx, pady = pady)

#clase DatePicker, tomado de https://github.com/maxverwiebe/CTkDatePicker y fue ligeramente modificado
class CTkDatePicker(ctk.CTkFrame):
    ...
    def build_calendar(self):
        ...
        self.calendar_frame = ctk.CTkFrame(self.popup, fg_color=CLARO, border_color=GRIS_CLARO3, border_width=1)
...
#clase CTkTable, tomado de: https://github.com/Akascape/CTkTable
#TODO: cambiar todas las tablas en el programa con una tabla propia y eliminar CTkTable
class CTkTable(ctk.CTkFrame):
    ...
    def __init__(
        ...
        **kwargs):
        ...
        self.border_color = ctk.ThemeManager.theme["CTkButton"]["border_color"] if border_color is None else border_color
        self.inside_frame = ctk.CTkFrame(self, border_width=0, fg_color="transparent")
        ...
        if self.colors[0] is None and self.colors[1] is None:
            ...
            if self.fg_color2==self.master.cget("fg_color"):
                self.fg_color2 = ctk.ThemeManager.theme["CTk"]["fg_color"]
        ...
        for i in range(self.rows):
            for j in range(self.columns):
                ...
                if self.write:
                    ...
                    self.frame[i,j] = ctk.CTkEntry(self.inside_frame,
                                                             font=self.font,
                                                             corner_radius=0,
                                                             **args)
                    if value is None:
                    ...
                    if value is None:
                        value = " "
                    self.frame[i,j] = ctk.CTkButton(self.inside_frame, background_corner_colors=corners,
                                                              font=self.font, 
    ...
    def insert(self, row, column, value, **kwargs):
        ...
        if self.write:
            self.frame[row,column].delete(0, ctk.END)
